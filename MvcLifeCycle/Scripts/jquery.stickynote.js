            ...
            if (note.settings.availableThemes.length != null && note.settings.availableThemes.length > 0
                ...
                //    }
                  ...
                  //  var noteBoxThemeOptions = $("<div class='" + themeBoxCssClass + "'></div>");
                    ...
                    for (var i = 0; i < note.settings.availableThemes.length; i++) {
                        ...
                        themeSelectOptions += "<input type=button class=sticky data-text='" + note.settings.availableThemes[i].text + "' data-value='" + note.settings.availableThemes[i].value + "'style='" + "background-color:" + note.settings.availableThemes[i].text + "' onclick='selectedtemplate(this)' />";
                    }
                   // noteBoxThemeSelect.append(themeSelectOptions);
                    noteBoxOptions.append(themeSelectOptions);
                ...
                //$(document).on("click", ".sticky", function () {
                    selectedtemplate = function (e) {
                        ...
                        note.settings.defaultTheme.value = e.attributes[3].nodeValue;
                        note.settings.defaultTheme.text = e.attributes[2].nodeValue;
                        note.settings.onThemeSelectionChange(note, noteBox);
                        noteBox.removeAttr("class");
                        noteBox.addClass(outerCssClass + " " + e.attributes[3].nodeValue);
                        e.stopPropagation();
                        //});
                    };
